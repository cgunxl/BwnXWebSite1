  ...
  return (
    <div ref={searchRef} className="relative max-w-2xl mx-auto">
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute top-full mt-2 w-full glass-card p-2 z-50 max-h-96 overflow-y-auto animate-fade-in">
          {suggestions.map((calcId) => (
            <button
              key={calcId}
              onClick={() => handleSelectCalculator(calcId)}
              className="w-full px-3 py-3 text-left hover:bg-surface-1 transition-colors flex items-center space-x-3 rounded-lg"
            >
              ...
              <div>
                ...
                <div className="text-sm text-text-muted">Click to open calculator</div>
              </div>
            </button>
          ))}
        </div>
